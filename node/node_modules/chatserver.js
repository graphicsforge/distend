    ...
    if ( channel!=undefined )
    {
      ...
      for ( var i=0; i<channels[socket.index].length; i++ )
      {
        if ( !channels[socket.index][i].length )
          continue;
        // channels[socket.index][i] should be in tsquery format, converting to boolean
  ...
  socket.addListener("connect", function() {
    ...
    for ( var i=0; i<clients.length; i++ )
      try {
        ...
        clients[i].emit("notify", "/add\tstatusBox\t"+clientaliases[socket.index]+"\n", "statusBox"); 
      } catch (error) {}
    console.log(clientaliases[index]+" connected from "+socket.remoteAddress);
  });

  socket.on("data", function(data) {
    if ( data.substr(0, 6)=="set_to" )
    {
      console.log("ignoring set to");
      data = data.slice(data.indexOf("\n")+1);
    }
//    console.log("["+clientaliases[socket.index]+"@"+socket.remoteAddress+"] \""+data+"\"");
        ...
        if ( channels[socket.index].indexOf( clientaliases[socket.index] )>=0 )
        {
          for ( var i=0; i<clients.length; i++ )
            try {
              ...
              clients[i].emit("notify", "/rename\tstatusBox\t"+clientaliases[socket.index]+" "+delimited[1]+"\n", "statusBox"); 
  clients[i].emit("notify", "rename\tstatusBox\t"+clientaliases[socket.index]+" "+delimited[1]+"\n", "statusBox"); 
            } catch (error) {}
    ...
    for ( var i=0; i<clients.length; i++ )
      if ( clients[i]!=undefined ){
        clients[i].emit("notify", clientaliases[socket.index]+"\tis no longer available\n", "statusBox");
          clients[i].emit("notify", "/remove\tstatusBox\t"+clientaliases[socket.index]+"\n", "statusBox");
      }
